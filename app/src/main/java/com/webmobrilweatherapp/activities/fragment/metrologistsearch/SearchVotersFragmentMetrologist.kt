...
import com.webmobrilweatherapp.databinding.FragmentSearchVotersMetrologistBinding

class SearchVotersFragmentMetrologist : Fragment(),
    MetrologistSearchprofileAdapter.AdapterInterface {
    lateinit var binding: FragmentSearchVotersMetrologistBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentSearchVotersMetrologistBinding.inflate(layoutInflater)
        ...
        accountViewModelMetrologist?.getsearchprofilesmetrologist(
            ...
            ?.observe(requireActivity()) {
                ...
                if (!requireActivity().isFinishing) {
                    ...
                    if (binding.recylerviewSearchVoter != null) {
                        if (it != null && it.success == true) {
                            if (it.data as ArrayList<Datum> != null && it.data.size > 0) {
                                binding.searchVoterMterologist.visibility = VISIBLE
                                ...
                                val layoutManager = LinearLayoutManager(requireContext(),
                                    ...
                                    false)
                                binding.recylerviewSearchVoter.layoutManager = layoutManager
                                binding.recylerviewSearchVoter.adapter =
                                    metrologistSearchprofileAdapter
                                //   Toast.makeText(context, it.message, Toast.LENGTH_LONG).show()
                            } else {
                                binding.searchVoterMterologist.visibility = GONE
                            }
