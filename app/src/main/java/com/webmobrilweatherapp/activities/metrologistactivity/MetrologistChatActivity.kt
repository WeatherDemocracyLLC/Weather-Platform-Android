...
import com.webmobrilweatherapp.activities.mediaadapter.chat.RetrofitInstance
import com.webmobrilweatherapp.databinding.ActivityMetrologistChatBinding
import com.webmobrilweatherapp.model.getchat.DataItem
import com.webmobrilweatherapp.viewmodel.ApiConstants
import com.pusher.client.Pusher
import com.pusher.client.PusherOptions
import com.pusher.pusherchat.MessageAdapter
...
class MetrologistChatActivity : AppCompatActivity() {
    lateinit var binding: ActivityMetrologistChatBinding
    ...
    private var lists: ArrayList<DataItem>? = ArrayList()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = DataBindingUtil.setContentView(this, R.layout.activity_metrologist_chat)
        ...
        adapter = MessageAdapter(this, list as ArrayList<DataItem>,userType)
        ...
        val linearLayoutManager: LinearLayoutManager = object : LinearLayoutManager(this) {
            override fun smoothScrollToPosition(
                ...
                val smoothScroller: LinearSmoothScroller =
                    object : LinearSmoothScroller(this@MetrologistChatActivity) {
    ...
    private fun setupPusher() {
        ...
        channel.bind("my-event") { channelName, eventName, data ->
            ...
            val dataItem = DataItem(
             ...
             runOnUiThread {
                 if ((jsonObject["from"].toString().toInt() ==currentUser) && (jsonObject["to"].toString().toInt() == useridMetrologist.toInt()))
                 {
                     adapter.add(dataItem)
                     binding.messageList.scrollToPosition(adapter.itemCount - 1);
                 }
                 else  if ((jsonObject["to"].toString().toInt() ==currentUser) && (jsonObject["from"].toString().toInt() == useridMetrologist.toInt())){
                     adapter.add(dataItem)
                     binding.messageList.scrollToPosition(adapter.itemCount - 1);
                 }
    ...
    private fun getmetrologistchat() {
        // ProgressD.showLoading(this, getResources().getString(R.string.logging_in))
        accountViewModelMetrologist?.getmetrologistchat(
            currentUser.toString(), "Bearer " + CommonMethod.getInstance(this).getPreference(
                AppConstant.KEY_token_Metrologist
            )
        )
            ?.observe(this) {
                ...
                if (it.data as ArrayList<DataItem>!=null&&it.data.size>0){
                    lists?.clear()
                    adapter = MessageAdapter(this, it.data as ArrayList<DataItem>,userType)
...
*/
    ...
    fun getImageUri(src: Bitmap, format: Bitmap.CompressFormat?, quality: Int): Uri? {
      /*  val os = ByteArrayOutputStream()
        ...
        src.compress(Bitmap.CompressFormat.PNG, 100, bytes)
    ...
    private fun sendMessage() {
        ...
        RetrofitInstance.retrofit.sendMessage(
            currentUser.toString(),
            binding.txtMessage.text.toString(),
            "1 ",formattedDatee,
            "Bearer " + CommonMethod.getInstance(this)
    ...
    private fun sendMessageImage() {
        ...
        RetrofitInstance.retrofit.sendMessageImagemtrologist(
            currentUserRequest, image, currentUserType,formattedDateee, "Bearer " + CommonMethod.getInstance(this)
                .getPreference(AppConstant.KEY_token_Metrologist)
        ).enqueue(object : Callback<String> {
            override fun onFailure(call: Call<String>?, t: Throwable?) {
                //resetInput()
    ...
    override fun onResume() {
        ...
        //getMessage()
        handler.post(getmetrologistchat)
    }

    override fun onPause() {
        super.onPause()
        handler.removeCallbacks(getmetrologistchat)
    }
    private val getmetrologistchat= object :Runnable{
        override fun run() {
            //call get api here
            getmetrologistchat()
        }
