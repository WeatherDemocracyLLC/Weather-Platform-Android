...
class MetrologistFollowersAdapter
    (private val context: Context, private  val List:List<FollowerListItem>):
    RecyclerView.Adapter<MetrologistFollowersAdapter.ViewHolder>() {
...
/*    interface SelectItem{
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MetrologistFollowersAdapter.ViewHolder {
        var itemView: View = LayoutInflater.from(context).inflate(R.layout.metrologistsearchprofile,parent,false)
        return  MetrologistFollowersAdapter.ViewHolder(itemView)
    }


    override fun onBindViewHolder(holder: MetrologistFollowersAdapter.ViewHolder, position: Int)
    {
        ...
        holder.itemView.setOnClickListener {
            CommonMethod.getInstance(context)
                .savePreference(AppConstant.KEY_IntsUserIdMetrologist, List.get(position).id.toString())


            ///if (List.get(position).userType.toString().equals("3"))
         ...
         ///  else if(List.get(position).userType.toString().equals("2"))
           else if(List.get(position).userType.toString().equals("3"))
            {

                var ids = List.get(position).id
                CommonMethod.getInstance(context).savePreference(AppConstant.KEY_IntsUserIdMetrologist, List.get(position).id.toString())
